Contributing to einrcore
=======

We're working hard to make *einrcore* the most powerful JavaScript library for working with einrcoin. Our goal is to have *einrcore* be a library that can be used by anyone interested in einrcoin, and to level expertise differences with great design and documentation.
...
These are some global design goals in einrcore that any change must adhere.
...
When possible, einrcore objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of einrcore methods. For example, calling `JSON.stringify` on an einrcore object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'einrcore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the einrcore object.

DO:
```javascript
var einrcore = require('../');
var PublicKey = einrcore.PublicKey;
```
DON'T:
```javascript
...
```sh
git remote add naveenedison git@github.com:naveenedison/einrcore.git
git pull --rebase naveenedison master
```
...
```
Finally go to [github.com/naveenedison/einrcore](https://github.com/naveenedison/einrcore) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of einrcore.

If you have any questions feel free to post them to
[github.com/naveenedison/einrcore/issues](https://github.com/naveenedison/einrcore/issues).
